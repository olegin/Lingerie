...
<IfModule mod_php5.c>
  php_flag engine off
	...
	public static function get_submitted_fields( $form, $lead, $display_empty = false, $use_text = false, $format = 'html', $use_admin_label = false, $merge_tag = '', $options = '' ) {
		...
		if ( $format == 'html' ) {
			$field_data = '<p>
Большое спасибо за регистрацию на Международную выставку нижнего белья, домашней одежды, купальников и чулочно-носочной продукции Lingerie Show-Forum, которая состоится в феврале 2016 года (чуть позже будут уточнены даты).
<p>
Для обмена на бейдж посетителя необходимо будет на регистрации предъявить распечатанный билет.';
		}
